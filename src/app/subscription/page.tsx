...
export default function SubscriptionPage() {
  ...
  const fetchSubscription = async () => {
    ...
    try {
      const res = await getMessSubscriptionDetails(mongoUser.messId);
      if (res.success) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!mongoUser?.messId || !mongoUser?._id) return;

    if (!senderPhone.trim()) {
      toast.error("প্রেনিক ফোন নম্বর লিখুন!");
      return;
    }
    if (!trxId.trim()) {
  ...
  return (
    <div className="w-full max-w-5xl mx-auto space-y-8 pb-16">
      {/* Header Banner */}
      <div className="bg-gradient-to-br from-indigo-900 via-indigo-800 to-slate-900 rounded-3xl p-8 text-white relative overflow-hidden shadow-xl border border-indigo-700/40">
        ...
        <div className="mt-6 pt-6 border-t border-indigo-700/50 flex flex-wrap items-center justify-between gap-4 relative z-10">
          {isSubActive ? (
            <div className="flex items-center gap-3 bg-emerald-500/20 border border-emerald-400/40 px-4 py-2.5 rounded-2xl backdrop-blur-md">
              <ShieldCheck className="w-6 h-6 text-emerald-400" />
              <div>
                <p className="text-xs font-black text-emerald-300">প্রিমিয়াম সাবস্ক্রিপশন সচল আছে</p>
                <p className="text-[11px] text-emerald-200/80 font-semibold">
                  মেয়াদ শেষ: {new Date(subData.expiresAt).toLocaleDateString('bn-BD', { day: 'numeric', month: 'long', year: 'numeric' })} (আর {daysLeft} দিন বাকি)
                </p>
              </div>
            </div>
          ) : (
            <div className="flex items-center gap-3 bg-rose-500/20 border border-rose-400/40 px-4 py-2.5 rounded-2xl backdrop-blur-md">
              <AlertCircle className="w-6 h-6 text-rose-400" />
              <div>
                <p className="text-xs font-black text-rose-300">সাবস্ক্রিপশন মেয়াদোত্তীর্ণ বা নিষ্ক্রিয়</p>
                <p className="text-[11px] text-rose-200/80 font-semibold">
          ...
          {pendingReq && (
            <div className="flex items-center gap-2 bg-amber-500/20 border border-amber-400/40 px-4 py-2.5 rounded-2xl text-xs text-amber-200 font-bold backdrop-blur-md">
      ...
      {/* Payment Number & Instruction Section */}
      <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-100 shadow-sm space-y-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-gradient-to-r from-rose-50 via-indigo-50 to-emerald-50 rounded-2xl p-6 border border-slate-200/60">
          <div className="space-y-1.5">
            ...
            <h2 className="text-2xl sm:text-3xl font-black text-slate-900 font-mono tracking-wider">
              01952321390
            </h2>
            <p className="text-xs text-slate-500 font-medium">
              উপরে উল্লেখিত নম্বরে bKash অথবা Nagad থেকে Send Money সম্পন্ন করুন এবং TrxID দিয়ে নিচে ফর্ম পূরণ করুন।
            </p>
          </div>
          ...
          <button
            ...
            className={`px-6 py-3.5 rounded-2xl font-black text-xs flex items-center gap-2 transition-all shadow-md ${
              copied
        ...
        {/* Subscription Request Submission Form */}
        <div className="pt-4 space-y-4">
          <div className="border-t border-slate-100 pt-6">
            <h3 className="text-base font-extrabold text-slate-900">২. পেমেন্ট ভেরিফিকেশন তথ্য দিন</h3>
          ...
          <form onSubmit={handleSubmit} className="space-y-5 max-w-xl">
            ...
            <div>
              <label className="block text-xs font-bold text-slate-700 mb-1.5">
                যে ফোন নম্বর থেকে টাকা পাঠিয়েছেন (Sender Number) *
              </label>
              <input
                type="text"
                required
